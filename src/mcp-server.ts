...
async function callTool(projectRoot: string, name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === 'codebone_skeleton') {
    ...
    if (stat.isDirectory()) return skeletonDirectory(projectRoot, target, { publicOnly: Boolean(args.publicOnly), maxFiles: Number(args.maxFiles ?? 50), budget: Number(args.budget ?? 12000), changedOnly: Boolean(args.changedOnly), mode: args.mode === 'summary' ? 'summary' : 'full' });
