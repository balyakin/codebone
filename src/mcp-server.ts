...
async function callTool(projectRoot: string, name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === 'codebone_skeleton') {
    ...
    if (stat.isDirectory()) return skeletonDirectory(projectRoot, target, { publicOnly: Boolean(args.publicOnly), publicApiOnly: mode === 'public_api', maxFiles: Number(args.maxFiles ?? 50), budget: Number(args.budget ?? 12000), changedOnly: Boolean(args.changedOnly), mode });
    return skeletonPath(projectRoot, target, { publicOnly: Boolean(args.publicOnly), publicApiOnly: mode === 'public_api', noImports: Boolean(args.noImports) || mode === 'public_api', budget: Number(args.budget ?? 12000) });
  }
  ...
  if (name === 'codebone_context') return buildContext(projectRoot, { goal: String(args.goal), path: String(args.path ?? '.'), budget: Number(args.budget ?? 8000), includeTests: args.includeTests !== false, changedOnly: Boolean(args.changedOnly), mode: ['architecture', 'overview', 'edit_prep'].includes(String(args.mode)) ? String(args.mode) as 'architecture' | 'overview' | 'edit_prep' : 'full', productionOnly: Boolean(args.productionOnly), testsOnly: Boolean(args.testsOnly), includeMocks: Boolean(args.includeMocks), includeConfig: Boolean(args.includeConfig), includeMigrations: Boolean(args.includeMigrations) });
...
const tools = [
  ...
  { name: 'codebone_skeleton', description: 'Get the structural skeleton of a source file or directory. Use mode:"public_api" to hide imports/private noise and keep classes, functions, routes, app keys, and tables; use mode:"summary" for large directories.', inputSchema: schema({ path: 'string', publicOnly: 'boolean', maxFiles: 'integer', budget: 'integer', changedOnly: 'boolean', mode: 'string' }, ['path']), outputSchema: outputSchema() },
  ...
  { name: 'codebone_context', description: 'Build a ranked context pack. Use mode:"overview" for human project summary, "architecture" for service graph, or "edit_prep" for minimal edit context and blast radius.', inputSchema: schema({ goal: 'string', path: 'string', budget: 'integer', includeTests: 'boolean', changedOnly: 'boolean', mode: 'string', productionOnly: 'boolean', testsOnly: 'boolean', includeMocks: 'boolean', includeConfig: 'boolean', includeMigrations: 'boolean' }, ['goal']), outputSchema: outputSchema() },
